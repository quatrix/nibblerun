...
fn test_constant_temperature() {
    let mut encoder = Encoder::new(300);
...
fn test_custom_interval() {
    ...
    let bytes = enc.to_bytes();
    let decoded_bytes = decode(&bytes, 60);
...
fn test_max_readings_65535() {
    ...
    let bytes = enc.to_bytes();
    let decoded_bytes = decode(&bytes, 300);
...
fn test_decode_truncated_header() {
    ...
    assert!(decode(&[], 300).is_empty());
    assert!(decode(&[0], 300).is_empty());
    assert!(decode(&[0; 9], 300).is_empty());

    // Exactly 10 bytes is valid header
    let mut header = [0u8; 10];
    // Set count to 0 - should return empty vec
    let decoded = decode(&header, 300);
...
fn test_1023_readings_same_interval() {
    ...
    let base_ts = 1761955455u64;
    let mut enc = Encoder::new(2000); // 2000 second interval
